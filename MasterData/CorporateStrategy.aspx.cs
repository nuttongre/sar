...
public partial class CorporateStrategy : System.Web.UI.Page
{
            ...
            if (!string.IsNullOrEmpty(mode))
            {
                switch (mode.ToLower())
                {
                    case "1":
                        ...
                        getddlYear(1);
                        btc.GenSort(txtSort, "CorporateStrategy", " And StudyYear = '" + ddlYearB.SelectedValue + "' ");
    ...
    private void bt_Save(string CkAgain)
    {
        ...
        if (String.IsNullOrEmpty(Request["mode"]) || Request["mode"] == "1")
        {
            ...
            i = Conn.AddNew("CorporateStrategy", "CorporateStrategyID, StudyYear, CorporateStrategyName, Detail, Sort, DelFlag, CreateUser, CreateDate, UpdateUser, UpdateDate", NewID, ddlYearB.SelectedValue, txtCorporateStrategy.Text, txtDetail.Text, txtSort.Text, 0, CurrentUser.ID, DateTime.Now, CurrentUser.ID, DateTime.Now);           
        ...
        if (Request["mode"] == "2")
        {
            i = Conn.Update("CorporateStrategy", "Where CorporateStrategyID = '" + Request["id"] + "' ", "StudyYear, CorporateStrategyName, Detail, Sort, UpdateUser, UpdateDate", ddlYearB.SelectedValue, txtCorporateStrategy.Text, txtDetail.Text, txtSort.Text, CurrentUser.ID, DateTime.Now);
