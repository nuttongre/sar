...
public partial class QACategory : System.Web.UI.Page
{
    BTC btc = new BTC();
    ...
    {       
            ...
            if (!string.IsNullOrEmpty(mode))
            {
                switch (mode.ToLower())
                {
                    case "1":
                        ...
                        getddlYear(1);
                        getddlScoreGroup();
                        btc.GenSort(txtSort, "QACategory", " And StudyYear = '" + ddlYearB.SelectedValue + "' ");
        ...
        txtQACategory.Attributes.Add("onkeyup", "Cktxt(0);");
    ...
    {
        //string StrSql = " Select a.QACategoryID, a.QACategoryName, a.Sort, IsNull(b.WeightScore, 0) WeightScore "
        //            + " From QACategory a Left Join ScoreGroup b On a.ScoreGroupID = b.ScoreGroupID "
        //            + " Where a.DelFlag = 0 And a.StudyYear = '" + ddlSearchYear.SelectedValue + "' ";

        string StrSql = @" Select a.QACategoryID, a.QACategoryName, a.Sort, IsNull(a.Score, 0) WeightScore 
                        From QACategory a 
                        Where a.DelFlag = 0 And a.StudyYear = '" + ddlSearchYear.SelectedValue + "' ";

        if (txtSearch.Text != "")
        {
            StrSql = StrSql + " And (a.QACategoryName Like '%" + txtSearch.Text + "%' Or a.Sort Like '%" + txtSearch.Text + "%')  ";
        }
    ...
    private void bt_Save(string CkAgain)
    {
        ...
        if (String.IsNullOrEmpty(Request["mode"]) || Request["mode"] == "1")
        {
            ...
            i = Conn.AddNew("QACategory", "QACategoryID, StudyYear, QACategoryName, Detail, Score, Sort, DelFlag, CreateUser, CreateDate, UpdateUser, UpdateDate", NewID, ddlYearB.SelectedValue, txtQACategory.Text, txtDetail.Text, txtScore.Text, txtSort.Text, 0, CurrentUser.ID, DateTime.Now, CurrentUser.ID, DateTime.Now);           

            if (CkAgain == "N")
            {
                Response.Redirect("QACategory.aspx?ckmode=1&Cr=" + i);    
            }
            if (CkAgain == "Y")
            {
                MultiView1.ActiveViewIndex = 1;
        ...
        if (Request["mode"] == "2")
        {
            i = Conn.Update("QACategory", "Where QACategoryID = '" + Request["id"] + "' ", "StudyYear, QACategoryName, Detail, Score, Sort, UpdateUser, UpdateDate", ddlYearB.SelectedValue, txtQACategory.Text, txtDetail.Text, txtScore.Text, txtSort.Text, CurrentUser.ID, DateTime.Now);
            Response.Redirect("QACategory.aspx?ckmode=2&Cr=" + i);    
        }
    }
    ...
    {
        if (String.IsNullOrEmpty(id)) return;
        if (btc.CkUseData(id, "QACategoryID", "QALink", "And DelFlag = 0"))
        {
            Response.Redirect("QACategory.aspx?ckmode=3&Cr=0"); 
        }
        else
        {
            Int32 i = Conn.Update("QACategory", "Where QACategoryID = '" + id + "' ", "DelFlag, UpdateUser, UpdateDate", 1, CurrentUser.ID, DateTime.Now);
            Response.Redirect("QACategory.aspx?ckmode=3&Cr=" + i); 
        }
    }
