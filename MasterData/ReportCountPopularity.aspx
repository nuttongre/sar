<%@ Page Language="C#" MasterPageFile="~/Master/MasterOriginal.master" CodeFile="ReportCountPopularity.aspx.cs"
    Inherits="ReportCountPopularity" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
        function popup(id, syear) {
            dialogBox.show('../PopUp/PopUpCountPopularity.aspx?id=' + id + '&syear=' + syear, 'รายชื่อโครงการ', '1000', '500', '#000', true, true);
        }
    </script>
    ...
    <asp:UpdatePanel ID="UpdatePanel1" runat="server">
        <ContentTemplate>
            <div id="pageDiv">
                ...
                <div class="headTable">
                    จำนวนโครงการตามค่านิยม 12 ประการ
                </div>
                            ...
                            <ItemTemplate>
                                <li class="<%#Eval("CountAc").ToString() == "0" ? "imgR" : "imgG" %>">
                                    ...
                                    <span class="<%#Eval("CountAc").ToString() == "0" ? "spantxtCountRed" : "spantxtCountNormal" %>"
                                        title="<%#Eval("CountAc")%>" onclick="popup('<%#Eval("PopularityID") %>', '<%# ddlYearB.SelectedValue %>');">
                                        <%#Eval("CountAc")%></span>
                                    <span class="spantxtStandardAct" title="<%#Eval("PopularityName") %>">
                                        <%#Eval("FullPopularityName") %></span>
                                    <hr /><br /></div>
                                </li>
                            </ItemTemplate>
                        </asp:Repeater>
