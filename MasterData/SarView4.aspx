<%@ Page Language="C#" MasterPageFile="~/Master/MasterOriginal.master" CodeFile="SarView4.aspx.cs"
    Inherits="SarView4" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function GotoActivity(id, id2)
        {
            var yearid = document.getElementById("<%=ddlYearB.ClientID%>").value;
            var ckview = document.getElementById("<%=rbtlView1.ClientID%>").checked ? 0 : 1;
            var deptid = document.getElementById("<%=ddlSearchDept.ClientID%>").value;
            var empid = document.getElementById("<%=ddlSearchEmp.ClientID%>").value;
            location.href="SarView5.aspx?year="+ yearid + "&pjid=" + id + "&idtid=" + id2 + "&ckview=" + ckview + '&deptid=' + deptid + '&empid=' + empid;
        }
    </script>
    ...
    <ajax:AjaxPanel ID="AjaxPanel1" runat="server">
                ...
                <ItemTemplate>
                    <a id="spnSvHead" onclick="SarView2('<%#Eval("SideCode")%>');">
                        <%#Eval("SideName")%></a>&nbsp;|&nbsp; <a id="spnSvHead" onclick="SarView3('<%#Eval("StandardCode")%>');">
                            <%#Eval("StandardName")%></a>&nbsp;|&nbsp; <span class="spnSvHeadAc">
                                <%#Eval("IndicatorsName")%></span>&nbsp;|
                </ItemTemplate>
            </asp:Repeater>
        </div>
        <br />
        ...
        <div style="padding: 0 0 0 40px; vertical-align: bottom;">
            <asp:Repeater ID="Repeater1" runat="server">
                <ItemTemplate>
                    <a style="cursor: pointer;" onclick="GotoActivity('<%#Eval("ProjectsCode")%>','<%#Eval("IndicatorsCode")%>');">
                        <%#Eval("FullName")%></a>
                </ItemTemplate>
            </asp:Repeater>
        </div>
