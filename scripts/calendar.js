function showCalendar(field, form){
if(form==undefined) form="";
addCalendar("name_"+field, field, field, form);
showCal("name_"+field);
}
// default settings
...
var titleWidth=96;
var titleMode=TitleMode.YearAndMonth;
var dayWidth=12;
var dayDigits=2;

var titleColor="#FFCC99";
var daysColor="#F1F1F1";
var bodyColor="#ffffff";
var dayColor="#ffffff";
var currentDayColor="#FF66FF";
var footColor="#F1F1F1";
...
function calHeader() {
  return "<table  bgcolor=\""+borderColor+"\" cellspacing=\"0\" cellpadding=\"0\"><tr><td><table cellspacing='1' cellpadding='3' border='0'>";
}

function calFooter() {
  return "<tr bgcolor='"+footColor+"'><td colspan='7' align='center'>"+span1(footColor)+"<b><a href='javascript:hideCal()' style='text-decoration:none; color:"+footFontColor+";'>["+cal_CloseText+"]</a>&nbsp;&nbsp;<a href='javascript:clearDate()' style='text-decoration:none; color:"+footFontColor+";'>["+cal_ResetText+"]</a></b>"+span2+"</td></tr></table></td></tr></table>";
}
...
var strDate=currentCal.form[currentCal.field].value;
    if (strDate!="" && calRE.test(strDate)) {
      var cd = getDateNumbers(strDate);
      d=new Date(Number(cd[0])-543,cd[1],cd[2]);
    ...
    if (isIE) {
      ...
      layers[cal].style.pixelTop=getTopIE(layers[currentCal.id],true)-100;
      layers[cal].style.pixelLeft=getLeftIE(layers[currentCal.id],true)+($get(currentCal.field).offsetWidth+60);
      layers[cal].innerHTML=calendar;
...
function pickDate(d) {
  var date=calFormat;
  date = date.replace(/yyyy/i, cY+543);
...
if (isN4) {
  ...
  document.writeln("<div id='"+cal+"' style='position:absolute; z-index:500'>&nbsp;</div>");
}
// ----
